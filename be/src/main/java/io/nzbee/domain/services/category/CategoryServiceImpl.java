...
public class CategoryServiceImpl implements ICategoryService {
	...
	public List<Category> findAll(String locale) {
    	List<CategoryProduct> lpc = categoryService.getAll();
    	return lpc.stream()
    ...
    @Override
 	...
 	public List<Category> findByParent(final String locale, Long parentCategoryId) {
    	List<CategoryProduct> lpc = categoryService.findByParent(CategoryVars.PRIMARY_HIERARCHY_CODE, CategoryVars.CATEGORY_TYPE_CODE_PRODUCT, parentCategoryId, locale);
    	return lpc.stream()
    			.map(pc -> createCategory(pc, locale))
    			.collect(Collectors.toList());
 	}
    ...
    @Override
  	...
  	public List<Category> findAllForLevel(final String locale, final Long level) {
     	List<CategoryProduct> lpc = categoryService.findByLevel(CategoryVars.PRIMARY_HIERARCHY_CODE, CategoryVars.CATEGORY_TYPE_CODE_PRODUCT, level, locale);
     	return lpc.stream()
     			.map(pc -> createCategory(pc, locale))
    			.collect(Collectors.toList());
  	}	
	...
	public Category findOneByDesc(String locale,  String categoryType, String categoryDesc) {
    	Optional<CategoryProduct> pc = categoryService.findByCategoryDesc(
    																categoryType, 
						    										categoryDesc, 
						    										locale
						    									   );
	...
	public List<Category> findAll(String locale, String categoryDesc, List<Brand> brands, List<Tag> tags) {
		...
		List<CategoryProduct> lc = categoryService.find(
